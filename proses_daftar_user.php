...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['btnSubmit'])) {
  ...
  $query = mysqli_query($conn, "SELECT * FROM user WHERE email = '$email' AND role = 'pembeli'");
  ...
  if ($count > 0) {
    echo "<script>
          ...
          header("location:register_user.php?info=terdaftar");
  } else {
    // jika email belum terdaftar maka simpan data ke dalam database
      mysqli_query($conn, "INSERT INTO user VALUES('$email', '$hashed_password', '', '', '', '', '', 'pembeli', '')");
      header("location:register_user.php?info=daftar");
  }
}
?>